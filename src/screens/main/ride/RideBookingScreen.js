...
export default function RideBookingScreen() {
  ...
  const handleBookRide = async () => {
    ...
    try {
      ...
      dispatch(setDriver(null)); // still searching
      ...
      Alert.alert(
        "Request failed",
        err?.data?.message || err?.message || "Could not request ride. Try again.",
      );
    }
  };

  const selected = RIDE_TYPES.find((r) => r.id === selectedRide);
  const basePrice = fareEstimate?.fare || fareEstimate?.price || 15;
        ...
        {stage === "search" ? (
          <View className="gap-4">
            <View className="flex-row gap-3 items-start">
              {/* dots */}
              <View className="items-center pt-3 gap-1">
                <View className="size-3 rounded-full bg-primary" />
                <View className="w-0.5 flex-1 min-h-10 bg-border" />
              ...
              <View className="flex-1 gap-2">
                ...
                <LocationAutocomplete
                  ...
                  biasCoords={currentLocation || pickup}
                  onSelect={(place) => {
                    dispatch(
                      setDestination({
                        coords: {
          ...
          <View className="gap-4">
              ...
              {RIDE_TYPES.map((ride) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <Text className="mt-2 text-sm font-inter-semibold text-foreground">
                      {ride.name}
                    </Text>
                    <Text
                      className={`
                        mt-1 text-sm font-inter-bold
            ...
            >
              <View className="flex-row items-center justify-between w-full px-1">
                <Text className="text-base font-inter-bold text-primary-foreground">
                  Book {selected?.name}
                </Text>
                <Text className="text-base font-inter-bold text-primary-foreground">
                  A$ {Math.round(basePrice * (selected?.multiplier || 1))}
                </Text>
              </View>
            </Button>
